...
% xlabel('Total points')
hl3 = subplot('Position',[0.06,0.55,0.15,0.15]);
...
%% precision compare
hpc = subplot('Position',[0.29,0.52,0.69,0.15]);
x_sbea = 1:3:48;
x_dlc = 2:3:48;
x_slp = 3:3:48;
y_sbea = [];
...
mean_raw_dlc = mean(y_dlc)';
sd_raw_sbea = std(y_sbea)';
sd_raw_dlc = std(y_dlc)';
N_raw_sbea = ones(size(y_sbea,2),1)*size(y_sbea,1);
N_raw_dlc = ones(size(y_dlc,2),1)*size(y_dlc,1);
group_data = [mean_raw_sbea,sd_raw_sbea,N_raw_sbea,...
    mean_raw_dlc,sd_raw_dlc,N_raw_dlc];


dsample_size = 100;
y_sbea = y_sbea(1:dsample_size:end,:);
y_dlc = y_dlc(1:dsample_size:end,:);
% y_slp = y_slp(1:dsample_size:end,:);

mean_sbea = mean(y_sbea);
mean_dlc = mean(y_dlc);

x_sbea_dlc = 1:16;
violinplot({y_dlc,y_sbea},x_sbea_dlc,'ShowData',false,'ShowMedian',false,...
    'Bandwidth',10,'ShowWhiskers',false,'ShowBox',false,...
    'ViolinColor',cbrewer2('Spectral',16));
hold on
plot([x_sbea_dlc-0.15;x_sbea_dlc+0.15],...
    [mean_raw_sbea';mean_raw_dlc'],'-k')
hold on 
plot(x_sbea_dlc-0.15,mean_raw_sbea','o','MarkerEdgeColor','k','MarkerFaceColor','w',...
    'MarkerSize',4)
hold on 
plot(x_sbea_dlc+0.15,mean_raw_dlc','o','MarkerEdgeColor','k','MarkerFaceColor','w',...
    'MarkerSize',4)
hold off
axis([0,17,-1,50])
box off
...
mean_raw_sbea = mean(y_sel_sbea)';
mean_raw_dlc = mean(y_sel_dlc)';
sd_raw_sbea = std(y_sel_sbea)';
sd_raw_dlc = std(y_sel_dlc)';
N_raw_sbea = ones(size(y_sel_sbea,2),1)*size(y_sel_sbea,1);
N_raw_dlc = ones(size(y_sel_dlc,2),1)*size(y_sel_dlc,1);
group_data = [mean_raw_sbea,sd_raw_sbea,N_raw_sbea,...
    mean_raw_dlc,sd_raw_dlc,N_raw_dlc];


dsample_size = 1;
y_sel_sbea = y_sel_sbea(1:dsample_size:end,:);
y_sel_dlc = y_sel_dlc(1:dsample_size:end,:);
% y_slp = y_slp(1:dsample_size:end,:);

mean_sbea = mean(y_sel_sbea);
mean_dlc = mean(y_sel_dlc);

% 
subplot('Position',[0.06,0.33,0.15,0.15])
cmap = cbrewer2('Paired',12);
plot(temppdfx,temppdfy,'-','Color',cmap(4,:),'LineWidth',1)
hold on
plot(cut_pos_x,cut_pos_y,'o','MarkerSize',5,'MarkerEdgeColor',cmap(5,:))
hold on
plot([cut_pos_x;cut_pos_x],[0,0,0;cut_pos_y],'-','Color',cmap(5,:))
hold off
...
box off
% close
subplot('Position',[start_plot_x,start_plot_y,box_width,box_height])
x_sbea_dlc = 1:16;
violinplot({y_sel_dlc,y_sel_sbea},x_sbea_dlc,'ShowData',false,'ShowMedian',false,...
    'Bandwidth',10,'ShowWhiskers',false,'ShowBox',false,...
    'ViolinColor',cbrewer2('Spectral',16));
hold on
plot([x_sbea_dlc-0.15;x_sbea_dlc+0.15],...
...
set(gca,'XTickLabel',[])
% far
% subplot('Position',[start_plot_x+inter_size+box_width,start_plot_y,box_width,box_height])
% set(gca,'TickDir','out')
% set(gca,'XTickLabel',[])
% set(gca,'YTickLabel',[])
...
% figure(2)
% hist(all_dist_data,128)

%% plot label
dot_x = [1:4,1:4,1:4,1:4];
dot_y = [7*ones(1,4),5*ones(1,4),3*ones(1,4),ones(1,4)];
body_parts = {...
...
subplot('Position',[0.24,0.22,0.74,0.08])
cmap = cbrewer2('Spectral',16);
for k = 1:16
    plot(dot_x(k),dot_y(k),'ko','MarkerFaceColor',cmap(k,:),'MarkerSize',4)
    hold on
end
