...
from SocialBehaviorAtlas.train_utils.vistr.util.box_ops import box_cxcywh_to_xyxy, generalized_box_iou, multi_iou
INF = 100000000
...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, num_frames : int = 36, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, num_out: int = 10):
        """Creates the matcher
...
def build_matcher(args):
    return HungarianMatcher(num_frames = args.num_frames, cost_class=args.set_cost_class,\
                            cost_bbox=args.set_cost_bbox, cost_giou=args.set_cost_giou, num_out=args.num_out)
