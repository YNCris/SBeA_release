function plot_skl_size(X,Y,Z,plotstr,linestr,markersize,linewidth)
%{
    绘制一帧对齐的骨架点
%}
%% 画点
...
% Z = XYZ(3:3:end);
for k = 1:size(X,1)
    plot3(X(k,1),Y(k,1),Z(k,1),'.','Color',plotstr,'MarkerSize',markersize)
    hold on
end
hold off
%% 划线
...
for l = 1:size(drawline,1)
    ...
    line(pts(:,1), pts(:,2), pts(:,3),'color',linestr,'linewidth',linewidth)
end
