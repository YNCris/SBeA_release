...
def correct_masks(result,dataset_info):
    ...
    return corrected_result
#%%
def sbea_eval_check(workroot,vismodelpath,evalmodelname,dataset_info,train_info,newvideopath):
    #%%
    ...
    #%%
    corrected_results = []
    for tempresult in all_result_list:
        print('correcting: ' + tempresult['videos'][0]['name'] + ' ......')
        tempcorrresult = correct_masks(tempresult,dataset_info)
        corrected_results.append(tempcorrresult)  
        with open(tempcorrresult['videos'][0]['name'][0:-4] + '-correctedresult.json', 'w', encoding='utf-8') as f:
            json.dump(tempcorrresult,f,indent=4)
    # #%% export masked videos and result.json
    # print('Export videos......')
    # corrjsonnamelist = get_files(folder,'correctedresult.json')
    # for tempname in corrjsonnamelist:
    #     with open(folder + '/' + tempname, 'r') as f:
    #         corrected_result = json.load(f)
    #     savevideopath = corrected_result['videos'][0]['name'][0:-4] + '-masks.mp4'
    #     writeobj = cv2.VideoWriter(savevideopath,cv2.VideoWriter_fourcc('m','p','4','v'),30,\
    #                              [corrected_result['annotations'][0]['segmentations'][0][0]['size'][1],\
    #                               corrected_result['annotations'][0]['segmentations'][0][0]['size'][0]])
    #
    #     readobj = cv2.VideoCapture(corrected_result['videos'][0]['name'])
    #     # select annotations
    #     selanno = copy.deepcopy(corrected_result['annotations'])
    #     #%
    #     lastgood = 0
    #     segcount = 0
    #     print('Exporting: ' + savevideopath)
    #     for k in tqdm(range(len(corrected_result['annotations'][0]['segmentations']))):
    #         readobj.set(cv2.CAP_PROP_POS_FRAMES,k)
    #         a,rawframe = readobj.read()
    #         rawmasksrles = copy.deepcopy(selanno[0]['segmentations'][k])
    #         tempgood = selanno[0]['searchgoodlist'][k]
    #
    #
    #         rawmasks = rle2mask(rawmasksrles)
    #
    ...
    #         # plt.show()
    #         writeobj.write(maskimg3)
    #     writeobj.release()
    #     print(savevideopath + ' finished!')


# %%
def sbea_eval_fast_check(workroot, vismodelpath, evalmodelname, dataset_info, train_info, newvideopath):
    # %%
    parser = argparse.ArgumentParser('VisTR inference script', parents=[get_args_parser()])
    args = parser.parse_args()
    ...
    for result in all_result_list:
        ...
        for k in range(160,180,5):#range(1,len(selanno['segmentations'])):
