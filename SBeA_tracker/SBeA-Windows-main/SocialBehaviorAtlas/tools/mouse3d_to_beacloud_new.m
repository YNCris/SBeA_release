...
close all
%% 设置路径
% mouse3dpath = 'Z:\hanyaning\multi_mice_test\Social_analysis\data/sbeaTestEnv/trainvideopath/videos2/mouse_3d';
mouse3dpath = ['Z:\hanyaning\multi_mice_test\Social_analysis\temp'];
savepath = [mouse3dpath,'/calibrationParas.mat'];
%% 读取标定文件
mouse3d_cell = cell(7,1);
for k = 1:7
    tempdata = load([mouse3dpath,'/PrimarySecondary',...
        num2str(k),'/calibrationSession.mat']);
    mouse3d_cell{k,1} = tempdata.calibrationSession;
end
%% 转换格式
camera_cell = cell(8,1);
cb_cell = cell(8,1);
...
camera_cell{5,1} = mouse3d_cell{4,1}.CameraParameters.CameraParameters2;
camera_cell{6,1} = mouse3d_cell{5,1}.CameraParameters.CameraParameters2;
camera_cell{7,1} = mouse3d_cell{6,1}.CameraParameters.CameraParameters2;
camera_cell{8,1} = mouse3d_cell{7,1}.CameraParameters.CameraParameters2;
...
cb_cell{5,1} = mouse3d_cell{4,1}.PatternSet.FullPathNames(2,:);
cb_cell{6,1} = mouse3d_cell{5,1}.PatternSet.FullPathNames(2,:);
cb_cell{7,1} = mouse3d_cell{6,1}.PatternSet.FullPathNames(2,:);
cb_cell{8,1} = mouse3d_cell{7,1}.PatternSet.FullPathNames(2,:);
%% 提取共享棋盘格
cb_short_cell = cell(8,1);
...
equal_mat = zeros(size(allframecell,1),8);
for k = 1:size(cb_short_cell,1)
...
%% 转换格式
caliParams.single_cam_mat = cell(8,1);
caliParams.single_rotation = cell(8,1);
caliParams.single_translation = cell(8,1);
for k = 1:size(camera_cell,1)
    ...
    if isempty(caliParams.single_rotation{1,1})
        caliParams.single_rotation{1,1} = camera_cell{1,1}.RotationMatrices(:,:,1);
        caliParams.single_translation{1,1} = camera_cell{1,1}.TranslationVectors(1,:);
    end
end
figure
cam_size = 50;
...
for k = 2:size(caliParams.single_rotation,1)
...
%% temp
cell_pose = cell(4,1);
for k = 1:size(cell_pose,1)
    R = caliParams.single_rotation{k,1};
    t = caliParams.single_translation{k,1};
    orientation = R';
    location = -t * orientation;
    cell_pose{k,1} = rigid3d(orientation,location);
    cam = plotCamera('AbsolutePose',cell_pose{k,1},'Opacity',0,'Size',20);
    hold on
end
hold off
