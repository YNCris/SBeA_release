...
case_no_dist_dsc = load([rootpath,'\panel_6\social_struct\recluster_data\',...
    'SBeA_data_sample_cell_20230201.mat']);
...
ylabel(h_cell{1,1},'y')
xlabel(h_cell{2,1},'x')
%% 6.5. distance map compare, and computational time comparison
dist_height = 0.12;
pos_y = 0.40;
subplot('Position',[0.06,pos_y,0.12,dist_height]);
zscore_embed_raw = cell2mat(case_dsc.data_sample_cell(:,4));
zscore_embed_raw = zscore_embed_raw(1:res_num:end,:);
distlist_raw = medfilt1(cellfun(@(x) min(x(:)),case_dsc.data_sample_cell(:,7)),3);
distlist_raw = distlist_raw(1:res_num:end,:);
normdistlist = round(mat2gray(distlist_raw)*255+1);
cmap = cbrewer2('Spectral',256);
cmaplist = cmap(normdistlist,:);
scatter(zscore_embed_raw(:,1),zscore_embed_raw(:,2),...
    mk_size*ones(size(zscore_embed_raw,1),1),cmaplist,'filled');
axis square
title('Distance map')
ylabel('Dimension 1')
xlabel('Dimension 2')
...
subplot('Position',[0.19,pos_y,0.12,dist_height]);
zscore_embed_no_dist = cell2mat(case_no_dist_dsc.data_sample_cell(:,4));
zscore_embed_no_dist = zscore_embed_no_dist(1:res_num:end,:);
distlist_no_dist = medfilt1(cellfun(@(x) min(x(:)),case_no_dist_dsc.data_sample_cell(:,7)),3);
distlist_no_dist = distlist_no_dist(1:res_num:end,:);
normdistlist = round(mat2gray(distlist_no_dist)*255+1);
cmap = cbrewer2('Spectral',256);
cmaplist = cmap(normdistlist,:);
scatter(zscore_embed_no_dist(:,1),zscore_embed_no_dist(:,2),...
    mk_size*ones(size(zscore_embed_no_dist,1),1),cmaplist,'filled');
axis square
title({'Distance map without','distance component'})
xlabel('Dimension 2')
set(gca,'TickDir','out')
set(gca,'XTickLabel',[],'YTickLabel',[])

% calculate map/body distance
unique_dist_names = unique(case_dsc.data_sample_cell(:,3));
raw_mb_dist = zeros(size(unique_dist_names,1),1);
nd_mb_dist = zeros(size(unique_dist_names,1),1);
for k = 1:size(unique_dist_names,1)
    % select data
    sel_name = unique_data_names{k,1};
    sel_idx_raw = strcmp(sel_name,case_dsc.data_sample_cell(:,3));
    sel_idx_no_dist = strcmp(sel_name,case_no_dist_dsc.data_sample_cell(:,3));
    sel_zscore_embed_raw = zscore_embed_raw(sel_idx_raw,:);
    sel_zscore_embed_no_dist = zscore_embed_no_dist(sel_idx_no_dist,:);
    sel_distlist_raw = distlist_raw(sel_idx_raw,:);
    sel_distlist_no_dist = distlist_no_dist(sel_idx_no_dist,:);
    % calculate map/body distance
    min_map_raw = zeros(size(sel_zscore_embed_raw,1),1);
    min_body_raw = zeros(size(sel_distlist_raw,1),1);
    min_map_no_dist = zeros(size(sel_zscore_embed_no_dist,1),1);
    min_body_no_dist = zeros(size(sel_distlist_no_dist,1),1);
    
    dist_mat_map_raw = squareform(pdist(sel_zscore_embed_raw));
    dist_mat_body_raw = squareform(pdist(sel_distlist_raw));
    dist_mat_map_no_dist = squareform(pdist(sel_zscore_embed_no_dist));
    dist_mat_body_no_dist = squareform(pdist(sel_distlist_no_dist));

    for m = 1:size(min_map_raw,1)
        sel_map_list_raw = dist_mat_map_raw(m,:);
        sel_map_list_raw(m) = [];
        min_map_raw(m) = min(sel_map_list_raw);

        min_idx = find(min_map_raw(m)==sel_map_list_raw);

        sel_body_list_raw = dist_mat_body_raw(m,:);
        sel_body_list_raw(m) = [];

        min_body_raw(m) = sel_body_list_raw(min_idx);
    end

    for m = 1:size(min_map_no_dist,1)
        sel_map_list_no_dist = dist_mat_map_no_dist(m,:);
        sel_map_list_no_dist(m) = [];
        min_map_no_dist(m) = min(sel_map_list_no_dist);

        min_idx = find(min_map_no_dist(m)==sel_map_list_no_dist);

        sel_body_list_no_dist = dist_mat_body_no_dist(m,:);
        sel_body_list_no_dist(m) = [];

        min_body_no_dist(m) = sel_body_list_no_dist(min_idx);
    end

    map_body_raw = min_map_raw./min_body_raw;
    map_body_no_dist = min_map_no_dist./min_body_no_dist;

    map_body_raw(isinf(map_body_raw)) = [];
    map_body_raw(isnan(map_body_raw)) = [];
    map_body_no_dist(isinf(map_body_no_dist)) = [];
    map_body_no_dist(isnan(map_body_no_dist)) = [];

    mean_map_body_raw = median(map_body_raw);
    mean_map_body_no_dist = median(map_body_no_dist);
    % append data
    raw_mb_dist(k,1) = mean_map_body_raw;
    nd_mb_dist(k,1) = mean_map_body_no_dist;
end

%%
subplot('Position',[0.37,pos_y,0.07,dist_height]);
mksize = 5;
mean_raw_mb = mean(raw_mb_dist);
mean_nd_mb = mean(nd_mb_dist);
std_raw_mb = std(raw_mb_dist)/sqrt(length(raw_mb_dist));
std_nd_mb = std(nd_mb_dist)/sqrt(length(raw_mb_dist));
for k = 1:size(raw_mb_dist,1)
    plot([1,2],[raw_mb_dist(k),nd_mb_dist(k)],'k')
    hold on
end
plot(1*ones(size(raw_mb_dist)),raw_mb_dist,'o',...
    'MarkerEdgeColor','k','MarkerFaceColor','w',...
    'MarkerSize',mksize)
hold on
plot(2*ones(size(nd_mb_dist)),nd_mb_dist,'o',...
    'MarkerEdgeColor','k','MarkerFaceColor','w',...
    'MarkerSize',mksize)
hold off
ylabel('Map/Body distance')
box off
axis([0.7,2.3,0.001,0.0035])
set(gca,'XTick',[1,2])
set(gca,'TickDir','out')
...
dist_time = 0.00002;%s
module_num = 1:10000;
res_train_time = 4*60;%s
res_pred_time = 1.6/200000;%s
raw_map_time = (module_num.^2)/2*dist_time/60;%min
res_map_time = (res_train_time+module_num*res_pred_time)/60;%min
subplot('Position',[0.32,0.29,0.12,0.07]);
plot(raw_map_time,'-','Color',[0.7,0.7,0.7],'LineWidth',1)
hold on
plot(res_map_time,'-','Color',cbrewer2('Paired',1),'LineWidth',1)
hold off
ylabel('Time [min]')
xlabel('Frame number')
box off
set(gca,'TickDir','out')
set(gca,'XTick',0:5000:module_num(end),'XTickLabelRotation',0)
set(gca,'YTick',0:5:max([raw_map_time,res_map_time]))
legend('No ResMLP','ResMLP','FontSize',6,'Location','northwest')
...
subplot('Position',[0.06,0.29,0.21,0.07]);
...
subplot('Position',[0.06,0.16,0.385,0.09]);
...
hcc = subplot('Position',[0.06,0.04,0.385,0.09]);
bd = 0.1;
side_bias = 0.2;
...
subplot('Position',[0.5,0.01,0.48,0.13])
