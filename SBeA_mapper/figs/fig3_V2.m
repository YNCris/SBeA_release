...
evalsize20 = load([rootpath,'\panel_41\body-multi-True-size20-eval.mat']);
evalnum22 = load([rootpath,'\panel_41\body-test20220602-multi-True-eval.mat']);
...
datarootpath = [rootpath,'\panel_41'];
fileFolder = fullfile(datarootpath);
dirOutput = dir(fullfile(fileFolder,'*3D.xls'));
err3dname = {dirOutput.name}';
dirOutput = dir(fullfile(fileFolder,'*pic.xls'));
...
errdata_cell_1w = cell(size(err3dname,1),3);
for k = 1:size(err3dname,1)
    %
    splname = split(err3dname{k,1},'_');
    errname = splname{1};
    ...
    tempxlspic = xlsread([datarootpath,'\',errpicname{k,1}]);
    %
    errdata_cell_1w{k,1} = errname;
    errdata_cell_1w{k,2} = tempxlspic;
    errdata_cell_1w{k,3} = tempxls3d;
end

featpath_hr = [rootpath,'\panel_43\feat_hr_20220822'];
featpath_hr_img = ['Z:\hanyaning\multi_mice_test\Social_analysis\data' ...
    '\disc1_shank3\sbea_shank3_disc1_20220822\datasets\id_images\' ...
    'body-multi-True\val'];
featpath_lr = [rootpath,'\panel_43\feat_lr_20220803'];
featpath_lr_img = ['Z:\hanyaning\multi_mice_test\Social_analysis\data' ...
    '\disc1_shank3\sbea_shank3_disc1_20220602\datasets\id_images\' ...
    'body-test20220803-multi-True\val'];
feat_hr_name = {};
feat_lr_name = {};
for k = 1:10
    fileFolder = fullfile(featpath_hr,['A',num2str(k)]);
    ...
    feat_hr_name = [feat_hr_name;[temphrfirstpath,temphrendpath,temphrname]];

    fileFolder = fullfile(featpath_lr,['A',num2str(k)]);
    dirOutput = dir(fullfile(fileFolder,'*_cam.mat'));
    temphrname = {dirOutput.name}';
    temphrpath = {dirOutput.folder}';
    temphrsplname = cellfun(@(x) split(x,'\'),temphrpath,'UniformOutput',false);
    ...
    temphrfirstpath = cellfun(@(x,y) x(1:(end-1-length(y))),temphrpath, ...
        temphrendpath,'UniformOutput',false);
    feat_lr_name = [feat_lr_name;[temphrfirstpath,temphrendpath,temphrname]];
end
%% plot canvas
h1 = figure(1);
...
set(gca,'XTickLabelRotation',0)
xlabel('Precision')
%% one week data compare
% number compare
prec_10 = diag(double(evaldata.cm))/1800;
prec_22 = diag(double(evalnum22.cm))/5400;
mean_prec_10 = mean(prec_10);
mean_prec_22 = mean(prec_22);
std_prec_10 = std(prec_10);
std_prec_22 = std(prec_22);
bar_w = 0.6;
line_w = 1;
bar_fc = 'w';
bar_c = cbrewer2('Dark2',2);
linc10 = bar_c(1,:);
linc22 = bar_c(2,:);
dot_w = 0.5;
rand('seed',2)
subplot('Position',[0.05,0.23,0.08,0.18])
bar(1,mean_prec_10,bar_w,'FaceColor',bar_fc,'EdgeColor',linc10,'LineWidth',line_w)
hold on
errorbar(1,mean_prec_10,std_prec_10,'Color',linc10,'LineWidth',line_w)
hold on
plot((dot_w*(rand([1,length(prec_10)])-0.5)+1),prec_10,'o',...
    'MarkerFaceColor','w','MarkerEdgeColor',linc10,'MarkerSize',3)
hold on
bar(2,mean_prec_22,bar_w,'FaceColor',bar_fc,'EdgeColor',linc22,'LineWidth',line_w)
hold on
errorbar(2,mean_prec_22,std_prec_22,'Color',linc22,'LineWidth',line_w)
hold on
plot((dot_w*(rand([1,length(prec_22)])-0.5)+2),prec_22,'o',...
    'MarkerFaceColor','w','MarkerEdgeColor',linc22,'MarkerSize',3)
hold off
axis([0.4,2.6,0,1.1])
set(gca,'XTick',1:2)
set(gca,'XTickLabel',{'10','22'})
xlabel('Mice number')
ylabel('Precision')
box off
set(gca,'TickDir','out')
...
% 50 compare 20
prec_50 = diag(double(evaldata.cm))/1800;
prec_20 = diag(double(evalsize20.cm))/1800;
mean_prec_50 = mean(prec_50);
mean_prec_20 = mean(prec_20);
std_prec_50 = std(prec_50);
std_prec_20 = std(prec_20);
bar_w = 0.6;
line_w = 1;
bar_fc = 'w';
bar_c = cbrewer2('Dark2',2);
...
dot_w = 0.5;
rand('seed',2)
subplot('Position',[0.15,0.23,0.08,0.18])
bar(1,mean_prec_50,bar_w,'FaceColor',bar_fc,'EdgeColor',linc50,'LineWidth',line_w)
hold on
errorbar(1,mean_prec_50,std_prec_50,'Color',linc50,'LineWidth',line_w)
hold on
plot((dot_w*(rand([1,length(prec_50)])-0.5)+1),prec_50,'o',...
    'MarkerFaceColor','w','MarkerEdgeColor',linc50,'MarkerSize',3)
hold on
bar(2,mean_prec_20,bar_w,'FaceColor',bar_fc,'EdgeColor',linc20,'LineWidth',line_w)
hold on
errorbar(2,mean_prec_20,std_prec_20,'Color',linc20,'LineWidth',line_w)
hold on
plot((dot_w*(rand([1,length(prec_20)])-0.5)+2),prec_20,'o',...
    'MarkerFaceColor','w','MarkerEdgeColor',linc20,'MarkerSize',3)
hold off
axis([0.4,2.6,0,1.1])
set(gca,'XTick',1:2)
set(gca,'XTickLabel',{'50','20'})
xlabel({'OF diameter','(cm)'})
box off
set(gca,'TickDir','out')
set(gca,'YTickLabel',[])

% day compare week
errlabel_1w = errdata_cell_1w(:,1);
errlenpic_1w = cellfun(@(x) 1-sum((x(:,2)-x(:,1)+1))/27000,errdata_cell_1w(:,2));
errlen3d_1w = cellfun(@(x) 1-sum((x(:,2)-x(:,1)+1))/27000,errdata_cell_1w(:,3));
mean_errlenpic = mean(errlenpic);
mean_errlenpic_1w = mean(errlenpic_1w);
std_errlenpic = std(errlenpic);
std_errlenpic_1w = std(errlenpic_1w);
mean_errlen3d = mean(errlen3d);
mean_errlen3d_1w = mean(errlen3d_1w);
std_errlen3d = std(errlen3d);
std_errlen3d_1w = std(errlen3d_1w);
xlist = [1,2,3.5,4.5];
bar_w = 0.6;
line_w = 1;
bar_fc = 'w';
...
dot_w = 0.5;
rand('seed',2)
subplot('Position',[0.27,0.23,0.18,0.18])
bar(xlist(1),mean_errlenpic,bar_w,'FaceColor',bar_fc,'EdgeColor',linc50,'LineWidth',line_w)
hold on
errorbar(xlist(1),mean_errlenpic,std_errlenpic,'Color',linc50,'LineWidth',line_w)
hold on
plot((dot_w*(rand([1,length(errlenpic)])-0.5)+xlist(1)),errlenpic,'o',...
    'MarkerFaceColor','w','MarkerEdgeColor',linc50,'MarkerSize',3)
hold on
bar(xlist(2),mean_errlenpic_1w,bar_w,'FaceColor',bar_fc,'EdgeColor',linc20,'LineWidth',line_w)
hold on
errorbar(xlist(2),mean_errlenpic_1w,std_errlenpic_1w,'Color',linc20,'LineWidth',line_w)
hold on
plot((dot_w*(rand([1,length(errlenpic_1w)])-0.5)+xlist(2)),errlenpic_1w,'o',...
    'MarkerFaceColor','w','MarkerEdgeColor',linc20,'MarkerSize',3)
hold on
bar(xlist(3),mean_errlen3d,bar_w,'FaceColor',bar_fc,'EdgeColor',linc50,'LineWidth',line_w)
hold on
errorbar(xlist(3),mean_errlen3d,std_errlen3d,'Color',linc50,'LineWidth',line_w)
hold on
plot((dot_w*(rand([1,length(errlen3d)])-0.5)+xlist(3)),errlen3d,'o',...
    'MarkerFaceColor','w','MarkerEdgeColor',linc50,'MarkerSize',3)
hold on
bar(xlist(4),mean_errlen3d_1w,bar_w,'FaceColor',bar_fc,'EdgeColor',linc20,'LineWidth',line_w)
hold on
errorbar(xlist(4),mean_errlen3d_1w,std_errlen3d_1w,'Color',linc20,'LineWidth',line_w)
hold on
plot((dot_w*(rand([1,length(errlen3d_1w)])-0.5)+xlist(4)),errlen3d_1w,'o',...
    'MarkerFaceColor','w','MarkerEdgeColor',linc20,'MarkerSize',3)
hold off
ylabel('Precision')
set(gca,'TickDir','out')
set(gca,'XTick',xlist)
axis([0.4,5.1,0.7,1.02])
box off
set(gca,'XTickLabel',{'Same day','Next week','Same day','Next week'})
xlabel('Images    3D poses')

%% features compare (50 cm,20 cm,16 body parts, random select 100 and mean)
% hr: hight resolution (20cm)
% lr: low resolution (50 cm)
feat_hr_mat = zeros(size(feat_hr_name,1),16);
feat_lr_mat = zeros(size(feat_lr_name,1),16);
tic
for k = 1:size(feat_hr_mat,1)
    tempcampath1 = feat_hr_name{k,1};
    tempcampath2 = feat_hr_name{k,2};
    tempcamname = feat_hr_name{k,3};
...
%         tempcamname(1:(end-8)),'.jpg']);
    ...
    for m = 1:size(tempcampos.feat_pos,1)
        ...
        feat_hr_mat(k,m) = all_meanselcam;
    end    
    if rem(k,100) == 0
        disp(k)
        toc
...
tic
for k = 1:size(feat_lr_mat,1)
    tempcampath1 = feat_lr_name{k,1};
    tempcampath2 = feat_lr_name{k,2};
    tempcamname = feat_lr_name{k,3};
    tempcam = load([tempcampath1,'\',tempcampath2,'\',tempcamname]);
    tempcampos = load([tempcampath1,'\',tempcampath2,'\', ...
        tempcamname(1:(end-8)),'.mat']);
%     tempcamimg = imread([featpath_hr_img,'\',tempcampath2,'\', ...
%         tempcamname(1:(end-8)),'.jpg']);
    ...
    for m = 1:size(tempcampos.feat_pos,1)
        ...
        all_meanselcam = all_meanselcam/4;
        feat_lr_mat(k,m) = all_meanselcam;
    end    
    if rem(k,100) == 0
        disp(k)
        toc
...
mean_hr_x = mean(feat_hr_mat);
mean_lr_x = mean(feat_lr_mat);
mean_hr_y = zeros(size(mean_hr_x));
mean_lr_y = zeros(size(mean_lr_x));
std_hr = std(feat_hr_mat);
std_lr = std(feat_lr_mat);

stat_hr = [zeros(size(mean_hr_x)),zeros(size(mean_hr_x)),zeros(size(mean_hr_x))];
stat_lr = [zeros(size(mean_lr_x)),zeros(size(mean_lr_x)),zeros(size(mean_lr_x))];
stat_hr(1:3:end) = mean_hr_x;
stat_lr(1:3:end) = mean_lr_x;
stat_hr(2:3:end) = std_hr;
stat_lr(2:3:end) = std_lr;
stat_hr(3:3:end) = size(feat_hr_mat,1);
stat_lr(3:3:end) = size(feat_lr_mat,1);


npoints = 1000;
kd_hr_x = zeros(npoints,size(feat_hr_mat,2));
kd_lr_x = zeros(npoints,size(feat_lr_mat,2));
kd_hr_y = zeros(npoints,size(feat_hr_mat,2));
kd_lr_y = zeros(npoints,size(feat_lr_mat,2));

for k = 1:size(kd_hr_y,2)
    
    xdlist = feat_hr_mat(:,k);
    temppdfx = linspace(min(xdlist),max(xdlist),1000);
    temppdfy = mat2gray(pdf(fitdist(xdlist,'kernel'),temppdfx))';
    kd_hr_x(:,k) = temppdfx';
    kd_hr_y(:,k) = temppdfy;
    
    find_pos = abs(temppdfx-mean_hr_x(k));
    min_pos = find_pos==min(find_pos);
    mean_hr_y(k) = temppdfy(min_pos);
    
    xdlist = feat_lr_mat(:,k);
    temppdfx = linspace(min(xdlist),max(xdlist),1000);
    temppdfy = mat2gray(pdf(fitdist(xdlist,'kernel'),temppdfx))';
    kd_lr_x(:,k) = temppdfx';
    kd_lr_y(:,k) = temppdfy;
    
    find_pos = abs(temppdfx-mean_lr_x(k));
    min_pos = find_pos==min(find_pos);
    mean_lr_y(k) = temppdfy(min_pos);
...
body_parts_16 = {...
    ...
    'Back','Root tail (Label)','Mid tail','Tip tail'};
subplot('Position',[0.5,0.33,0.43,0.08])
for k = x_hlr
    show_y_lr = k-kd_lr_y(:,k)/shri_size;
    show_y_hr = kd_lr_y(:,k)/shri_size+k;
    plot(show_y_lr,kd_lr_x(:,k),'Color',linc50)
    hold on
    plot([k-mean_lr_y(k)/shri_size-line_len,k],[mean_lr_x(k),mean_lr_x(k)], ...
        'Color',linc50)
    hold on
    plot(show_y_hr,kd_lr_x(:,k),'Color',linc50)
    hold on
    plot([k+mean_lr_y(k)/shri_size+line_len,k],[mean_lr_x(k),mean_lr_x(k)], ...
        'Color',linc50)
    hold on
    plot(k,mean_lr_x(k),'ko',...
        'MarkerFaceColor','w','MarkerEdgeColor',linc50,...
        'MarkerSize',mk_size)
    hold on
end
hold off
axis([0.5,16.5,0,maxy])
set(gca,'TickDir','out')
ylabel('Normalized CAM')
set(gca,'Xtick',1:16)
set(gca,'XTickLabel',[])
box off
subplot('Position',[0.5,0.23,0.43,0.08])
for k = x_hlr
    show_y_lr = k-kd_hr_y(:,k)/shri_size;
    show_y_hr = kd_hr_y(:,k)/shri_size+k;
    plot(show_y_lr,kd_hr_x(:,k),'Color',linc20)
    hold on
    plot([k-mean_hr_y(k)/shri_size-line_len,k],[mean_hr_x(k),mean_hr_x(k)], ...
        'Color',linc20)
    hold on
    plot(show_y_hr,kd_hr_x(:,k),'Color',linc20)
    hold on
    plot([k+mean_hr_y(k)/shri_size+line_len,k],[mean_hr_x(k),mean_hr_x(k)], ...
        'Color',linc20)
    hold on
    plot(k,mean_hr_x(k),'ko',...
        'MarkerFaceColor','w','MarkerEdgeColor',linc20,...
        'MarkerSize',mk_size)
    hold on
end
hold off
...
set(gca,'TickDir','out')
set(gca,'Xtick',1:16)
set(gca,'XTickLabel',body_parts_16)
box off
%% quantified by mouse, CAM
unique_hr_mice = unique(feat_hr_name(:,2));
mouse_hr_cam = zeros(size(unique_hr_mice,1),size(feat_hr_mat,2));
for k = 1:size(unique_hr_mice,1)
    sel_idx = strcmp(feat_hr_name(:,2),unique_hr_mice{k});
    mean_data = mean(feat_hr_mat(sel_idx,:));
    mouse_hr_cam(k,:) = mean_data;
end

unique_lr_mice = unique(feat_lr_name(:,2));
mouse_lr_cam = zeros(size(unique_lr_mice,1),size(feat_lr_mat,2));
for k = 1:size(unique_lr_mice,1)
    sel_idx = strcmp(feat_lr_name(:,2),unique_lr_mice{k});
    mean_data = mean(feat_lr_mat(sel_idx,:));
    mouse_lr_cam(k,:) = mean_data;
end

%% plot new cam compare
body_parts_16 = {...
    'Nose','Left ear','Right ear','Neck',...
    'Left front limb','Right front limb','Left hind limb','Right hind limb',...
    'Left front paw','Right front paw','Left hind paw','Right hind paw',...
...
show_seq = [14,1:13,15:16];
mean_cam_50 = mean(mouse_lr_cam(:,show_seq));
std_cam_50 = std(mouse_lr_cam(:,show_seq));
mean_cam_20 = mean(mouse_hr_cam(:,show_seq));
std_cam_20 = std(mouse_hr_cam(:,show_seq));
subplot('Position',[0.5,0.33,0.38,0.08])
bar(1:16,mean_cam_50,bar_w,'FaceColor',bar_fc,'EdgeColor',linc50,'LineWidth',line_w)
hold on
for k = 1:16
    errorbar(k,mean_cam_50(k),std_cam_50(k),...
        'Color',linc50,'LineWidth',line_w,'CapSize',3)
    hold on
    plot(k+0.3,mouse_lr_cam(:,show_seq(k)),'o',...
    'MarkerFaceColor','w','MarkerEdgeColor',linc50,'MarkerSize',3)
    hold on
end
hold off
axis([0.5,16.5,0.06,0.28])
set(gca,'TickDir','out')
ylabel('Normalized CAM')
set(gca,'Xtick',1:16)
set(gca,'XTickLabel',[])
box off
subplot('Position',[0.5,0.23,0.38,0.08])
bar(1:16,mean_cam_20,bar_w,'FaceColor',bar_fc,'EdgeColor',linc20,'LineWidth',line_w)
hold on
for k = 1:16
    errorbar(k,mean_cam_20(k),std_cam_20(k),...
        'Color',linc20,'LineWidth',line_w,'CapSize',3)
    hold on
    plot(k+0.3,mouse_lr_cam(:,show_seq(k)),'o',...
    'MarkerFaceColor','w','MarkerEdgeColor',linc20,'MarkerSize',3)
    hold on
...
hold off
axis([0.5,16.5,0.07,0.24])
set(gca,'YTick',[0.1,0.15,0.2])
set(gca,'TickDir','out')
set(gca,'Xtick',1:16)
set(gca,'XTickLabel',body_parts_16(show_seq))
box off
