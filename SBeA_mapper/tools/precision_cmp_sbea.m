...
sbeapath = 'F:\multi_mice_test\Social_analysis\methods_compare\data\pred_sbea';
...
for k = 1:size(gtvideonames,1)
    ...
    %% load sbea data
    sbea_1_csv = importdata([sbeapath,'\single_1_',tempname,'.csv']);
    sbea_2_csv = importdata([sbeapath,'\single_2_',tempname,'.csv']);
    dlc_1_data = sbea_1_csv.data;
    dlc_1_data(:,1) = [];
    dlc_1_data(:,3:3:end) = [];
    dlc_2_data = sbea_2_csv.data;
    dlc_2_data(:,1) = [];
    dlc_2_data(:,3:3:end) = [];
...
%% save
save([savepath,'\sbea_err_id_cell.mat'],'err_id_cell');
save([savepath,'\sbea_err_nid_cell.mat'],'err_nid_cell');
disp('save finished!')